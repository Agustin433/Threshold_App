"""One-shot migration: backfill DJ_drop_height_cm = 30 on existing DJ evaluations.
...
historical DJ test and silently produced NaN for all of them. The gym's coach
confirmed every DJ test on file so far was actually performed at a 30cm box,
so this script backfills that known value for the affected rows only.
...
def main() -> None:
    ...
    df.loc[target_mask, "DJ_drop_height_cm"] = DROP_HEIGHT_CM

    df.to_csv(csv_path, index=False)
    print(f"Filas migradas a DJ_drop_height_cm={DROP_HEIGHT_CM}: {affected}")
