...
from tests.test_jump_analysis_cohort_zscore import (
    _build_two_cohort_frames,
    _expected_z,
    build_valid_cohort_frames,
)
...
class PrepareJumpDfAcceptsProfileDfTest(unittest.TestCase):
    ...
    def test_prepare_jump_df_accepts_profile_df_and_uses_cohort(self):
        jump_df, profile_df, heights = build_valid_cohort_frames()
        result = _prepare_jump_df(jump_df.copy(), profile_df=profile_df)

        expected = _expected_z(heights, heights[0])
        actual = result.loc[result["Athlete"] == "Cohorte 0", "SJ_Z"].iloc[0]
