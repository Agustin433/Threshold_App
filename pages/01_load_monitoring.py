...
if rdf is None or not acwr_dict:
    st.info("Todavía no hay datos de carga procesados. Cargá RPE + Tiempo desde la pantalla principal y luego volvé a esta página.")
else:
    ...
    acwr_col = "ACWR_EWMA" if selected_method.startswith("EWMA") else "ACWR_Classic"
    acwr_label = "ACWR EWMA" if acwr_col == "ACWR_EWMA" else "ACWR Clásico"
    ...
    metrics[0].metric("sRPE última sesión", f"{last_session['sRPE'].iloc[-1]:.0f} UA" if not last_session.empty else "-")
    metrics[1].metric("RPE última sesión", f"{last_session['RPE'].iloc[-1]:.1f}" if not last_session.empty else "-")
    metrics[2].metric(acwr_label, f"{last_acwr['ACWR_EWMA'].iloc[-1]:.2f}" if not last_acwr.empty else "-")
    metrics[3].metric("Monotonía", f"{last_mono['Monotonia'].iloc[-1]:.2f}" if not last_mono.empty else "-")
    ...
    if not last_acwr.empty:
        ...
        if acwr_value > 1.5:
            st.error(f"{acwr_label}: {acwr_value:.2f}. Zona alta; conviene bajar densidad o exposición aguda.")
        elif acwr_value > 1.3:
            st.warning(f"{acwr_label}: {acwr_value:.2f}. Hay que monitorear el próximo bloque de 48 horas.")
        elif acwr_value < 0.8:
            st.info(f"{acwr_label}: {acwr_value:.2f}. Señal de subcarga o recuperación prolongada.")
        else:
            st.success(f"{acwr_label}: {acwr_value:.2f}. La carga reciente está dentro de la zona objetivo.")
    ...
    with chart_left:
        if acwr_df is not None and not acwr_df.empty:
            chart_key = "ACWR_EWMA" if acwr_col == "ACWR_EWMA" else "ACWR_Classic"
            st.plotly_chart(
                chart_acwr(acwr_df, athlete, chart_key, theme=theme),
    ...
    if raw_df is not None:
        ...
        if athlete_col and athlete in raw_df[athlete_col].astype(str).values:
            st.markdown("### Volumen por patrón")
